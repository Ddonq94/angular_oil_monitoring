...
processFile(ev, ed?){
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.http
          .post<DataTablesResponse>(
            ...
            'https://app05.sahara-group.com/pjh-api/public/api/dt/stations',
  ...
  validateNew(){
    ...
    if(this.bank.length >= 2){
      voter++;

    }

    if(this.acctNum.length >= 2){
      voter++;
  ...
  validateEdit(){
    ...
    if(this.ebank.length >= 2){
      voter++;

    }

    if(this.eacctNum.length >= 2){
      voter++;
  ...
  editModal(edContent, node){
    ...
    this.ebank = node.bank;
    this.eacctNum = node.account_number;
  ...
  edit(){
    ...
    this.globalservice.editNode(this.edID,nodeObj,'stations')
    .then(data => {
      ...
      if(this.res.status == "success"){
        this.inputSuccess = "Station Edited Successfully";
  ...
  del(node){
    ...
    this.globalservice.delNode(node.id,"stations")
    .then(data => {
      ...
      if(this.res.status == "success"){
        this.inputSuccess = "Station Deleted Successfully";
        setTimeout(function () {
  ...
  saveNew(){
    ...
    this.globalservice.newNode(nodeObj,'stations')
    .then(data => {
      ...
      if(this.res.status == "success"){
        this.inputSuccess = "Station Created Successfully";
        setTimeout(function () {
        that.modalService.dismissAll();
